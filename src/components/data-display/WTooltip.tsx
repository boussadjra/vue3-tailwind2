    ...
    setup(props, { slots }) {
        ...
        return () => (
            <div class={`relative group max-w-max`} ref={tooltip} >
                ...
                {show.value && <span class={`absolute flex  ${positions[props.position].outerFlex} inset-0 pointer-events-none whitespace-nowrap ${positions[props.position].margin}   z-10 `}>
                    <span class={`flex  font-normal ${positions[props.position].innerFlex} text-white transition transform ${positions[props.position].translate}    scale-90 group-hover:scale-100`}>
                        ...
                        <svg class={`text-gray-900 dark:text-black-100 h-2 w-2 ${positions[props.position].arrow}`} viewBox="0 0 255 255">
